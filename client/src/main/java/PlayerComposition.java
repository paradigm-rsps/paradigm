public class PlayerComposition {
	...
	public void changeSex(boolean var1) {
		if (this.isFemale != var1) {
			this.update(null, this.bodyColors, var1, -1);
		}
	}
	...
	void setHash() {
		...
		for (var5 = 0; var5 < 12; ++var5) {
			...
			if (this.equipment[var5] >= 256) {
				this.hash += this.equipment[var5] - 256;
			}
		}

		if (this.equipment[0] >= 256) {
			this.hash += this.equipment[0] - 256 >> 4;
		}

		if (this.equipment[1] >= 256) {
			this.hash += this.equipment[1] - 256 >> 8;
		}

		for (var5 = 0; var5 < 5; ++var5) {
			this.hash <<= 3;
			this.hash += this.bodyColors[var5];
		}
	...
	public Model getModel(SequenceDefinition var1, int var2, SequenceDefinition var3, int var4) {
		if (this.npcTransformId != -1) {
			...
			if (var1 != null && (var1.shield >= 0 || var1.weapon >= 0)) {
				...
				if (var1.shield >= 0) {
					var5 += var1.shield - this.equipment[5] << 40;
					var7[5] = var1.shield;
				}

				if (var1.weapon >= 0) {
					var5 += var1.weapon - this.equipment[3] << 48;
					var7[3] = var1.weapon;
				}
			}
